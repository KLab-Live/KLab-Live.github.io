...
이 문서는 Bode Lab의 `단일 루프` 탭과 `캐스케이드` 탭에서 사용하는 주요 수식과 지표의 의미를 정리한 공개 설명 자료이다. 입력값은 화면에 표시되는 단위를 기준으로 하며, 계산에는 필요한 경우 SI 단위로 환산한다.
...
| $\lvert X\rvert$ | 복소 응답 $X(j\omega)$의 크기 |
| $\angle X$ | 복소 응답 $X(j\omega)$의 위상 |
| dB | $20\log_{10}\lvert X\rvert$ |
| $T_s$ | 전류 루프 제어 주기 |
| $T_{sw}$ | 속도 루프 샘플링 주기 |
...
$BW_i$는 전류 루프 폐루프 대역폭이다. 전류 루프 대역폭을 명확히 판단하기 어려운 경우에는 `폐루프 Ti 사용` 모드로 해석하는 것이 더 적합하다.
...
시간 영역 추정치는 $0 < PM \le 70^\circ$, $BW > 0$ 범위에서 의미 있게 해석한다. 오버슈트와 정착 시간은 추가로 $0 < \zeta < 1$인 부족감쇠 영역에서 사용하는 값이다.
...
두 탭 모두 로그 주파수 그리드를 사용한다. 화면의 곡선은 0.1 Hz부터 각 루프의 샘플링 한계까지 촘촘히 평가한 값으로 그린다.
